...
- [第四部分：集群篇 —— 分布式推理与未来趋势](#第四部分集群篇--分布式推理与未来趋势)
  - [第十六章：切分巨人：张量、流水线与上下文并行](#第十六章切分巨人张量流水线与上下文并行)
    ...
    - [第五节：打破序列墙：上下文并行 (Context Parallelism)](#第五节打破序列墙上下文并行context-parallelism)
  ...
  - [第十八章：打通经脉：大模型推理中的网络通信与高速互联](#第十八章打通经脉大模型推理中的网络通信与高速互联)
    - [第一节：模式、数据量与指标影响大汇总](#第一节模式数据量与指标影响大汇总)
    - [第二节：单机内的血脉：PCIe、NVLink 与 NVSwitch](#第二节单机内的血脉pcie-nvlink-与-nvswitch)
    - [第三节：跨机桥梁：RDMA 与 InfiniBand](#第三节跨机桥梁rdma-与-infiniband)
    - [第四节：网络条件与并行模式的“最佳拍档”](#第四节网络条件与并行模式的最佳拍档)
  ...
  - [第二十章：云原生底座：Kubernetes 适配分布式推理的优化与演进](#第二十章云原生底座kubernetes-适配分布式推理的优化与演进)
...
- [第五部分：前沿篇 —— 现代 LLM 推理的复杂场景与极限挑战](#第五部分前沿篇--现代-llm-推理的复杂场景与极限挑战)
  - [第二十一章：计算与推理范式的突变：MoE 架构与 CoT 思考](#第二十一章计算与推理范式的突变moe-架构与-cot-思考)
    - [第一节：混合专家模型（MoE）的混乱](#第一节混合专家模型moe的混乱)
    - [第二节：思考模式（CoT）与 KV Cache 爆炸](#第二节思考模式cot与-kv-cache-爆炸)
  - [第二十二章：大模型“操作系统”化：智能体、检索增强与动态适配](#第二十二章大模型操作系统化智能体检索增强与动态适配)
...
#### 第二节：单机内的血脉：PCIe、NVLink 与 NVSwitch

在单台服务器内部，多张 GPU 之间的互联技术经历了巨大的演进：
1.  **PCIe (Peripheral Component Interconnect Express)**:
    *   **特点**：传统的通用总线。目前主流的 PCIe 4.0/5.0 带宽相对有限（例如 PCIe 5.0 x16 单向带宽约为 64 GB/s）。
    *   **局限**：在 TP（张量并行）这种需要极高频、海量数据同步的场景下，PCIe 会成为严重的瓶颈。
2.  **NVLink**:
    *   **特点**：NVIDIA 专门为 GPU 直连开发的高速互联技术。它允许 GPU 之间直接读取对方的内存（P2P），绕过 CPU。
    *   **带宽**：每一代的 NVLink 带宽都在飙升（如 NVLink 4.0 在 H100 上可提供高达 900 GB/s 的双向总带宽），远超 PCIe。
3.  **NVSwitch**:
    *   **特点**：NVLink 解决了两两直连的问题，而 NVSwitch 则是一个“交换机”，将单机内的 8 张（或更多）GPU 连接成一个全互联的网络。
    *   **作用**：它保证了任意两张 GPU 之间都能以 NVLink 的最高速度进行通信，是实现高效张量并行（TP）的物理基石。

#### 第三节：跨机桥梁：RDMA 与 InfiniBand

当分布式推理跨越物理节点（Multi-host）时，传统的以太网和 TCP/IP 协议栈由于经过内核拷贝，延迟和 CPU 消耗极大，无法满足 LLM 推理的需求。
1.  **RDMA (Remote Direct Memory Access)**:
    *   **特点**：远程直接内存访问。它允许一台机器的网卡直接访问另一台机器的 GPU 显存，实现“零拷贝”（Zero-copy）。
    *   **收益**：极大地降低了网络延迟，解放了 CPU 算力。
2.  **InfiniBand (IB)**:
    *   **特点**：一种专为高性能计算（HPC）设计的网络技术。
    *   **优势**：提供极高的带宽（如 400Gbps、800Gbps）和极低的延迟（微秒级），并且是“无损网络”（Lossless Network），通过基于信用的流控机制避免丢包重传。
3.  **RoCE (RDMA over Converged Ethernet)**:
    *   **特点**：在传统的以太网之上运行 RDMA。
    *   **折中**：成本比 IB 低，但需要配置复杂的无损以太网环境（如 PFC、ECN），否则在拥塞时性能会急剧下降。

#### 第四节：网络条件与并行模式的“最佳拍档”

不同的分布式并行模式，对网络通信的要求有着天壤之别。合理匹配网络技术与并行模式，是架构设计的核心：
...
### 第二十一章：计算与推理范式的突变：MoE 架构与 CoT 思考
...
### 第二十二章：大模型“操作系统”化：智能体、检索增强与动态适配
