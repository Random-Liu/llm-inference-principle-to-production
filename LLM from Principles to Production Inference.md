...
在深入研究 vLLM 等 $N$ 个词的向量**同时**与模型的权重矩阵（如 $W_Q, W_K, W_V$ 以及 FFN 的矩阵）进行乘法运算。这在数学上是标准的**矩阵-矩阵乘法（GEMM）**。
*   **注意力计算**：由于 Decoder-Only 架构的因果掩码（Causal Mask）限制，**每个词只能与它前面的词**计算注意力，生成一个下三角的 $N \times N$ 注意力分数矩阵。
*   **总计算复杂度**：单轮时间复杂度为 $O(L \cdot (N \cdot d^2 + N^2 \cdot d))$，其中 $L$ 为模型层数，$d$ 为隐藏层维度。
