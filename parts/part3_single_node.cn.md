...
- [第十章：精度降维：KV Cache 量化（FP8/INT8）](#第十章精度降维kv-cache-量化fp8int8)
  ...
  - [第三节：INT8 与 FP8：截然不同的量化范式](#第三节int8-与-fp8截然不同的量化范式)
  - [第四节：动态与静态：与模型权重量化的对比](#第四节动态与静态与模型权重量化的对比)
...
在标准的推理中，K 和 V 都是以 16 位（FP16 或 BF16）的精度存储的，每个元素占 2 字节。而 **KV Cache 量化** 的核心思想，就是将它们压缩为 8 位（FP8 或 INT8），使每个元素仅占 1 字节，从而将显存占用直接砍半。
...
在 NVIDIA H100 等现代 GPU 上，硬件已经原生支持 FP8 的张量计算，这种量化转换的计算开销几乎可以忽略不计。因此，用微不足道的计算代价，换取显存占用减半、搬运速度翻倍，成为了现代高性能推理引擎的标配。更重要的是，将 KV Cache 压缩到 8-bit（FP8/INT8）带来的精度损失通常不到 0.5%，几乎无损，却能让系统承载的并发用户量直接翻倍。
