...
*   **Stopping New Requests (Gateway Switching)**: Before upgrading a node, mark it as unschedulable (Cordon). At this moment, rely on the Endpoint controller's asynchronous synchronization. By invoking a `preStop` hook (or native K8s `sleep` action) before containers receive the `SIGTERM` signal, a pause is enforced. This ensures that the Pod's endpoint has enough time to be removed from the upstream gateway or load balancers (LB) for a seamless, natural traffic cut-off.
