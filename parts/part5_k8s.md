...
- [Chapter 21: Racing Against Time: Model Distribution and Cold Start Optimization](#chapter-21-racing-against-time-model-distribution-and-cold-start-optimization)
  - [Section 1: Separation of Image and Weights: Choice of Model Formats](#section-1-separation-of-image-and-weights-choice-of-model-formats)
    - [Format War: Why Safetensors Became the Most Popular Format](#format-war-why-safetensors-became-the-most-popular-format)
  - [Section 2: Mass Data Distribution: Packaging Protocols and Pod Mounting](#section-2-mass-data-distribution-packaging-protocols-and-pod-mounting)
    - [1. Packaging Protocols: Git LFS vs OCI Artifact](#1-packaging-protocols-git-lfs-vs-oci-artifact)
    - [2. How Models Enter Pods](#2-how-models-enter-pods)
    - [3. P2P and Streaming: Accelerating Weight Distribution and Cold Starts](#3-p2p-and-streaming-accelerating-weight-distribution-and-cold-starts)
  - [Section 3: VRAM Loading Optimization: Three Schools of Data Paths and Trade-offs](#section-3-vram-loading-optimization-three-schools-of-data-paths-and-trade-offs)
    - [1. Route 1: `mmap` + Traditional Copy (Single-Threaded Page Triggered)](#1-route-1-mmap--traditional-copy-single-threaded-page-triggered)
    - [2. Route 2: Multi-threaded `pread` + Pinned Memory (e.g., Run:ai Streamer)](#2-route-2-multi-threaded-pread--pinned-memory-eg-runai-streamer)
    - [3. Route 3: GPUDirect Storage (GDS) (Ultimate Hardware Direct Path)](#3-route-3-gpudirect-storage-gds-ultimate-hardware-direct-path)
- [Chapter 22: Tentacles Reaching into the Motherboard: DRA and Hardware Topology Aware Scheduling](#chapter-22-tentacles-reaching-into-the-motherboard-dra-and-hardware-topology-aware-scheduling)
  ...
  - [Section 2: Evolution: DRA (Dynamic Resource Allocation) and Resource Management Paradigm Revolution](#section-2-evolution-dra-dynamic-resource-allocation-and-resource-management-paradigm-revolution)
    - [1. What is DRA?](#1-what-is-dra)
    - [2. Motivations and Pain Points Solved by DRA](#2-motivations-and-pain-points-solved-by-dra)
  - [Section 3: Single-Node Battle: Facing Hardware Locality](#section-3-single-node-battle-facing-hardware-locality)
    - [1. Ultimate Solution: Solving Three Topology Pain Points with DRA](#1-ultimate-solution-solving-three-topology-pain-points-with-dra)
  - [Section 4: Beyond Single Node: Cluster-Level Network Topology and Multi-Machine Synergy](#section-4-beyond-single-node-cluster-level-network-topology-and-multi-machine-synergy)
    - [1. Problem Restated: Random Collisions of Cluster Network Topology (Problem 4)](#1-problem-restated-random-collisions-of-cluster-network-topology-problem-4)
    - [2. Solution: Native K8s Topology-Aware Scheduling](#2-solution-native-k8s-topology-aware-scheduling)
    - [3. Configuration Example: Rack Alignment via Pod Affinity](#3-configuration-example-rack-alignment-via-pod-affinity)
- [Chapter 23: Breaking Silos: LeaderWorkerSet and Distributed LLM Orchestration](#chapter-23-breaking-silos-leaderworkerset-and-distributed-llm-orchestration)
  ...
  - [Section 2: NCCL: The Fragile Lifeline of Distributed Inference](#section-2-nccl-the-fragile-lifeline-of-distributed-inference)
    - [1. What is NCCL and Why is it Important?](#1-what-is-nccl-and-why-is-it-important)
    - [2. Why is NCCL So Fragile?](#2-why-is-nccl-so-fragile)
    - [3. What to Do When NCCL Breaks?](#3-what-to-do-when-nccl-breaks)
  - [Section 3: The Birth of LeaderWorkerSet: Primitives Tailored for AI](#section-3-the-birth-of-leaderworkerset-primitives-tailored-for-ai)
    - [1. Core Policies: Taming Tightly Coupled Lifecycles](#1-core-policies-taming-tightly-coupled-lifecycles)
    - [2. Upgrade Policy and Behavior](#2-upgrade-policy-and-behavior)
    - [3. Predictable Naming Strategy and Network Identity](#3-predictable-naming-strategy-and-network-identity)
    - [4. Exclusive Topology Scheduling: "Reserved Seating" for Multiple Groups](#4-exclusive-topology-scheduling-reserved-seating-for-multiple-groups)
...
- [Chapter 24: All-or-Nothing: Gang Scheduling and Resource Deadlocks](#chapter-24-all-or-nothing-gang-scheduling-and-resource-deadlocks)
  ...
  - [Section 3: Gang Scheduling Implementations in Kubernetes](#section-3-gang-scheduling-implementations-in-kubernetes)
    - [1. Standalone Schedulers](#1-standalone-schedulers)
    - [2. Coscheduling Plugin (via Scheduler Framework)](#2-coscheduling-plugin-via-scheduler-framework)
    - [3. Native Kubernetes Gang Scheduling (Native Support, Evolving)](#3-native-kubernetes-gang-scheduling-native-support-evolving)
  - [Section 4: Kueue: Job Queuing and Quota Management](#section-4-kueue-job-queuing-and-quota-management)
    - [1. Problems Solved by Kueue](#1-problems-solved-by-kueue)
    - [2. Relationship with Gang Scheduling](#2-relationship-with-gang-scheduling)
    - [3. Typical Use Cases for Kueue](#3-typical-use-cases-for-kueue)
- [Chapter 25: Breathing of the Pool: Pod and Node Autoscaling in LLM Inference](#chapter-25-breathing-of-the-pool-pod-and-node-autoscaling-in-llm-inference)
  - [Section 1: Pod Autoscaling: Shifting from Metrics to Events](#section-1-pod-autoscaling-shifting-from-metrics-to-events)
    - [1. Failure of Native HPA: Why CPU/Memory Metrics Fail in LLM Scenarios](#1-failure-of-native-hpa-why-cpumemory-metrics-fail-in-llm-scenarios)
    - [2. Custom Metrics and KEDA: Event-Driven Scaling](#2-custom-metrics-and-keda-event-driven-scaling)
    - [3. Group Scaling: LWS + HPA Synergy](#3-group-scaling-lws--hpa-synergy)
  - [Section 2: Node Autoscaling: Just-In-Time Provisioning](#section-2-node-autoscaling-just-in-time-provisioning)
    - [1. Two Approaches to Node Autoscaling: Cluster Autoscaler and Karpenter](#1-two-approaches-to-node-autoscaling-cluster-autoscaler-and-karpenter)
    - [2. Node Autoscaling Challenges: Latency and Atomicity](#2-node-autoscaling-challenges-latency-and-atomicity)
    - [3. Pragmatic Solutions](#3-pragmatic-solutions)
- [Chapter 26: Operations and Upgrades: Continuity vs. Heavy Assets](#chapter-26-operations-and-upgrades-continuity-vs-heavy-assets)
  - [Section 1: Traditional K8s Paradigms: Misalignment with LLM Inference](#section-1-traditional-k8s-paradigms-misalignment-with-llm-inference)
    - [1. Insufficient Graceful Shutdown Time and Long Connections](#1-insufficient-graceful-shutdown-time-and-long-connections)
    - [2. Eviction and Version Dilemmas in All-or-Nothing Scenarios](#2-eviction-and-version-dilemmas-in-all-or-nothing-scenarios)
    - [3. Physical Cold Start Prolongs Upgrade Cycles and Costs](#3-physical-cold-start-prolongs-upgrade-cycles-and-costs)
  - [Section 2: Seamless Upgrades: Engineering Practices](#section-2-seamless-upgrades-engineering-practices)
    - [1. Connection Draining and Extra-Long Graceful Termination](#1-connection-draining-and-extra-long-graceful-termination)
    - [2. Addressing All-or-Nothing: Group Upgrades vs. Blue-Green](#2-addressing-all-or-nothing-group-upgrades-vs-blue-green)
  - [Section 3: Upgrade Cold Starts: State Retention Challenges](#section-3-upgrade-cold-starts-state-retention-challenges)
    - [1. Model Weights: Reusing Local Cache](#1-model-weights-reusing-local-cache)
    - [2. KV Cache: From Scratch vs. State Inheritance](#2-kv-cache-from-scratch-vs-state-inheritance)
- [Part 5 Summary: Core Contradictions and Breakthroughs in LLM Orchestration](#part-5-summary-core-contradictions-and-breakthroughs-in-llm-orchestration)
  - [Contradiction 1: Heavy "Soft State" — The Cost of Creation and Reconstruction](#contradiction-1-heavy-soft-state--the-cost-of-creation-and-reconstruction)
  - [Contradiction 2: Rigid "Compute Topology" — Collision Between Tight and Loose Coupling](#contradiction-2-rigid-compute-topology--collision-between-tight-and-loose-coupling)
