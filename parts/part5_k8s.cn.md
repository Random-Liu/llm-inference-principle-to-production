...
*   **跨节点 P2P 共享（P2P Sharing）**：利用集群内的高速内网，让尚未拥有权重文件的节点从已持有文件的邻居节点（Peer）处拉取数据，极大化解中心化存储的带宽瓶颈。
*   **流式按需拉取（Streaming / Lazy Loading）**：将大文件切分为数据块（Chunks）。Pod 启动时仅拉取几兆字节的元数据（Metadata）即可瞬间就绪。当推理引擎真正访问某一权重分片时，底层文件系统才会以缺页中断（Page Fault）的方式按需拉取对应的数据块，从而消灭全量下载的等待时间。
...
*   **原理**：将模型权重伪装打包为标准的 **OCI Artifact**（OCI 规范产物），并在容器运行时（containerd）层面引入 Nydus 作为快照器（Snapshotter），配合 Dragonfly 劫持网络流量并提供拓扑 P2P 传输能力。
    ...
    *   **跨节点 P2P 共享（P2P Sharing）**：由 Dragonfly 赋能。Dragonfly 在每个计算节点上部署 Peer 节点。当多台服务器同时拉取权重时，Peer 节点间会进行拓扑感知并互相“投喂”已拉取的分片。
...
*   **场景考量**：得益于公有云内部极其强悍、高冗余的基础设施，以及对象存储数十 GB/s 的海量出口带宽，这类架构极度轻巧，免去了庞大缓存集群的容量规划与运维，甚至可以 **“免用本地缓存（No Local Cache）”** 依旧保障极致的加载性能。然而，在带宽受限或成本敏感的私有化算力中心，这种直接流式读取会瞬间压垮网络。因此，在私有化场景下，结合 **P2P 与本地缓存** 的方案依然是架构师们不可或缺的考量。

> [!IMPORTANT]
> **考量 P2P 分发引入的决定性因素**
> 
> 架构师在评估是否引入 **P2P** 机制时，环境本身的存储基础设施强度是核心的决定性因素：
> 
> *   **在私有化 / 离线隔离环境（On-prem / Air-gapped）下**：由于内部 Registry 或对象存储的并发带宽极其受限，P2P 机制是避免“惊群效应（Thundering Herd）”拉垮整个中心存储集群的 **绝对刚需**。
> *   **在公有云环境（Public Cloud）下**：由于 S3 / GCS 等云厂商存储的吞吐量极高，P2P 的必要性相对下降。此时它主要作为一种**成本与延迟手段**，用于减少跨 AZ / Egress 的流量费用，并加速大规模快速扩缩容（Auto-scaling）时新节点的冷启动时间。
