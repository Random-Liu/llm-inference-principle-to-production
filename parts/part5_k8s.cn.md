...
目前对于这个“物理冷启动”难题，业界并没有完美的终极方案，主要依靠以下两种务实（甚至有些 Hack）的手段来缓解：

##### ① 预留容量缓冲（Capacity Buffer / Balloon Pods）
*   **原理**：既然临时买机器太慢，那就提前买好“备用”。过去的做法是部署一些低优先级的“狗腿子 Pod”（Balloon Pods，里面只跑 sleep），强行占住一部分 GPU 节点。当真实的推理任务（高优先级）来临时，Scheduler 会瞬间踢死狗腿子，让真实任务“秒级接管”。
*   **进化**：由于手写狗腿子 Pod 容易引发级联抖动，目前正走向正规化。例如 GKE 推出了声明式的 **`CapacityBuffer` API**，在底层内存中生成虚拟的占位需求来预热节点；或者利用 **Kueue 的 Provisioning Request** 机制，强迫 Autoscaler 先去买机器，等机器彻底 Ready 后再放行任务，将等待时间隔离在业务队列中。

##### ② 加速节点启动（Accelerate Node Startup）
*   **原理**：尽可能压缩上述冷启动 1-4 步的时间。
*   **现状**：**目前并没有特别完美的“一锤定音”的办法**。业界在各显神通：使用预装了驱动和基础镜像的自定义 AMI/OS 镜像；利用 Dragonfly + Nydus 进行 P2P 和流式按需加载模型；甚至探索利用 `gVisor` 等技术对已经加载了权重的 GPU 内存进行“快照切片”瞬时恢复。但这些手段都存在较高的技术门槛或局限性，无法完全消灭物理开销。

因此，在实际的工程落地中，通常是 **“容量缓冲兜底实时突发，加速启动缩短缓冲补充时间”** 的组合拳，以空间（成本）换取时间。
