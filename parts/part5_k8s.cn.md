...
## 目录
- [第二十章：当“松耦合”遇上“紧耦合”：K8s 与 LLM 的生命周期碰撞](#第二十章当松耦合遇上紧耦合k8s-与-llm-的生命周期碰撞)
...
- [第二十一章：与时间赛跑：模型分发与冷启动优化](#第二十一章与时间赛跑模型分发与冷启动优化)
  - [第一节：镜像与权重分离：模型格式的选择](#第一节镜像与权重分离模型格式的选择)
    - [格式争霸：为什么 Safetensors 成为了最流行的格式？](#格式争霸为什么-safetensors-成为了最流行的格式)
  - [第二节：海量数据分发：打包协议与 Pod 挂载的博弈](#第二节海量数据分发打包协议与-pod-挂载的博弈)
    - [1. 打包协议的博弈：Git LFS 与 OCI Artifact 的各擅胜场](#1-打包协议的博弈git-lfs-与-oci-artifact-的各擅胜场)
    - [2. 决战冷启动：模型究竟是如何进入 Pod 的？](#2-决战冷启动模型究竟是如何进入-pod-的)
    - [3. P2P 与流式拉取：极致加速权重分发与冷启动](#3-p2p-与流式拉取极致加速权重分发与冷启动)
  - [第三节：显存加载优化：三种流派的数据通路与权衡](#第三节显存加载优化三种流派的数据通路与权衡)
    - [1. 方案一：`mmap` + 传统拷贝 (单线程页触发)](#1-方案一mmap--传统拷贝-单线程页触发)
    - [2. 方案二：多线程 `pread` + 锁页内存 (如 Run:ai Streamer)](#2-方案二多线程-pread--锁页内存-如-runai-streamer)
    - [3. 方案三：GPUDirect Storage (GDS) (终极硬件直通)](#3-方案三gpudirect-storage-gds-终极硬件直通)
- [第二十二章：伸进主板的触角：DRA 与硬件拓扑感知调度](#第二十二章伸进主板的触角dra-与硬件拓扑感知调度)
  - [第一节：拓扑黑洞：为什么标量计数在分布式推理中失效？](#第一节拓扑黑洞为什么标量计数在分布式推理中失效)
  - [第二节：进化之路：DRA（动态资源分配）与资源管理范式革命](#第二节进化之路dra动态资源分配与资源管理范式革命)
    - [1. 什么是 DRA？](#1-什么是-dra)
    - [2. DRA 的多重动机与解决的痛点](#2-dra-的多重动机与解决的痛点)
  - [第三节：单机战场：直面硬件局部性（Hardware Locality）](#第三节单机战场直面硬件局部性hardware-locality)
    - [1. 终极方案：用 DRA 优雅解决三大拓扑痛点](#1-终极方案用-dra-优雅解决三大拓扑痛点)
  - [第四节：跨越单机：集群级网络拓扑与多机协同](#第四节跨越单机集群级网络拓扑与多机协同)
    - [1. 痛点重述：集群级网络拓扑的“随机碰撞”（对应问题 4）](#1-痛点重述集群级网络拓扑的随机碰撞对应问题-4)
    - [2. 解决之道：K8s 原生的拓扑感知调度](#2-解决之道k8s-原生的拓扑感知调度)
    - [3. 配置示例：用 Pod 亲和性实现机柜级对齐](#3-配置示例用-pod-亲和性实现机柜级对齐)
- [第二十三章：打破孤岛：LeaderWorkerSet 与大模型分布式编排](#第二十三章打破孤岛leaderworkerset-与大模型分布式编排)
  - [第一节：传统控制器的局限：为什么微服务范式不再适用？](#第一节传统控制器的局限为什么微服务范式不再适用)
  - [第二节：NCCL：分布式推理的脆弱生命线](#第二节nccl分布式推理的脆弱生命线)
    - [1. NCCL 是干什么的，为什么重要？](#1-nccl-是干什么的为什么重要)
    - [2. 为什么 NCCL 极易损坏？](#2-为什么-nccl-极易损坏)
    - [3. NCCL 坏掉之后需要怎么办？](#3-nccl-坏掉之后需要怎么办)
  - [第三节：LeaderWorkerSet 的诞生：专为 AI 打造的编排基元](#第三节leaderworkerset-的诞生专为-ai-打造的编排基元)
    - [1. 核心策略：驯服紧耦合的生命周期](#1-核心策略驯服紧耦合的生命周期)
    - [2. 升级策略（Upgrade Policy）与行为](#2-升级策略upgrade-policy与行为)
    - [3. 可预测的命名策略与网络标识（Predictable Naming）](#3-可预测的命名策略与网络标识predictable-naming)
    - [4. 独占拓扑调度：多组并发时的“对号入座”](#4-独占拓扑调度多组并发时的对号入座)
  - [第四节：实战演练：用 LWS 部署分布式 vLLM 服务](#第四节实战演练用-lWS-部署分布式-vllm-服务)
- [第二十四章：全有或全无：Gang Scheduling 与资源死锁](#第二十四章全有或全无gang-scheduling-与资源死锁)
  ...
  - [第三节：Gang Scheduling 在 Kubernetes 中的实现](#第三节gang-scheduling-在-kubernetes-中的实现)
    - [1. 独立的调度器（Standalone Schedulers）](#1-独立的调度器standalone-schedulers)
    - [2. Coscheduling 插件（基于 Scheduler Framework）](#2-coscheduling-插件基于-scheduler-framework)
    - [3. Kubernetes 原生 Gang Scheduling（原生支持，演进中）](#3-kubernetes-原生-gang-scheduling原生支持演进中)
  - [第四节：Kueue：作业排队与配额管理的艺术](#第四节kueue作业排队与配额管理的艺术)
    - [1. Kueue 解决什么问题？](#1-kueue-解决什么问题)
    - [2. 它与 Gang Scheduling 的关系](#2-它与-gang-scheduling-的关系)
    - [3. Kueue 的典型应用场景](#3-kueue-的典型应用场景)
- [第二十五章：动态伸缩：Pod 与 Node 的 Autoscaling 艺术](#第二十五章动态伸缩pod-与-node-的-autoscaling-艺术)
  - [第一节：Pod Autoscaling：从指标到事件的进化](#第一节pod-autoscaling从指标到事件的进化)
    - [1. 原生 HPA 的死穴：为什么 CPU/Mem 指标在 LLM 场景下失效？](#1-原生-hpa-的死穴为什么-cpumem-指标在-llm-场景下失效)
    - [2. Custom Metrics 与 KEDA](#2-custom-metrics-与-keda)
    - [3. 强强联合：LWS + HPA 的组级伸缩](#3-强强联合lws--hpa-的组级伸缩)
  - [第二节：Node Autoscaling：从“买套餐”到“去菜市场”](#第二节node-autoscaling从买套餐到去菜市场)
    - [1. 节点扩缩容的两种流派：Cluster Autoscaler 与 Karpenter](#1-节点扩缩容的两种流派cluster-autoscaler-与-karpenter)
    - [2. 节点扩容的暗礁：滞后性与原子性](#2-节点扩容的暗礁滞后性与原子性)
    - [3. 虽不完美但务实的解法](#3-虽不完美但务实的解法)
- [第二十六章：运维与升级：业务连续性与重资产的博弈](#第二十六章运维与升级业务连续性与重资产的博弈)
  - [第一节：传统 K8s 升级范式在 LLM 场景的“水土不服”](#第一节传统-k8s-升级范式在-llm-场景的水土不服)
    - [1. 优雅终止时间（`terminationGracePeriodSeconds`）与长连接的冲突](#1-优雅终止时间terminationgraceperiodseconds与长连接的冲突)
    - [2. “All-or-Nothing” 视角的驱逐与版本风险](#2-all-or-nothing-视角的驱逐与版本风险)
    - [3. 物理冷启动拉长升级周期与资源冗余成本](#3-物理冷启动拉长升级周期与资源冗余成本)
  - [第二节：实现无感知的优雅升级](#第二节实现无感知的优雅升级)
    - [1. 连接流干（Connection Draining）与超长优雅终止](#1-连接流干connection-draining与超长优雅终止)
    - [2. 针对“All-or-Nothing”：组级升级与蓝绿翻转](#2-针对all-or-nothing组级升级与蓝绿翻转)
  - [第三节：深水区挑战：Upgrade 场景的“冷启动”与状态保持](#第三节深水区挑战upgrade-场景的冷启动与状态保持)
    - [1. 模型权重：本地缓存的复用可能](#1-模型权重本地缓存的复用可能)
    - [2. KV Cache：从“无中生有”到“状态继承”](#2-kv-cache从无中生有到状态继承)
- [本部分总结：LLM 在 K8s 中的两大核心矛盾与破局之道](#本部分总结llm-在-k8s-中的两大核心矛盾与破局之道)
  - [矛盾一：沉重的“软状态”（Heavy Soft-State）—— 启动与重建的饱和轰炸](#矛盾一沉重的软状态heavy-soft-state-启动与重建的饱和轰炸)
  - [矛盾二：严苛的“计算拓扑”（Rigid Compute-Topology）—— 紧耦合与松耦合的生死碰撞](#矛盾二严苛的计算拓扑rigid-compute-topology-紧耦合与松耦合的生死碰撞)
...
## 第二十四章：全有或全无：Gang Scheduling 与资源死锁
...
## 第二十五章：动态伸缩：Pod 与 Node 的 Autoscaling 艺术
...
### 第二节：Node Autoscaling：从“买套餐”到“去菜市场”
...
为了在不影响业务连续性的前提下完成节点升级，业界通常采用“软驱逐”和智能流量调度的组合拳。

#### 1. 连接流干（Connection Draining）与超长优雅终止
为了实现真正的无感知升级，必须在流量切换和进程终止上做精细化控制：
*   **停止新请求（网关切流）**：这是标准的微服务做法。在准备升级节点时，首先将其标记为不可调度（Cordon）。此时需要配合 `preStop` Hook，通知上层网关或 Service Mesh 停止向该节点分发新请求，实现优雅的流量摘除。值得注意的是，Kubernetes 引入了原生的 **[`sleep` 动作](https://kubernetes.io/docs/concepts/containers/container-lifecycle-hooks/)**（在 lifecycle hook 中），允许我们在容器收到终止信号前原生暂停一段时间，以确保网络端点有足够时间从负载均衡器中摘除，而不再需要像以前那样在 `preStop` 中写丑陋的 shell 脚本进行 `sleep`。
