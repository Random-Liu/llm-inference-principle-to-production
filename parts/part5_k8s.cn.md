...
*   **原理**：在数据卷（PVC）底层铺设分布式缓存网络。Fluid 充当 K8s 层面的数据编排控制面，而 Alluxio 则作为数据面将各个节点的本地 SSD 聚合成集群内部的分布式缓存池。
...
##### ③ 对应流派三（Pod 级胶水）的演进：Object-storage-native 模式（直接流式读取）
*   **代表组合**：**`Run:ai Model Streamer / Mountpoint for Amazon S3 / AIBrix`**
*   **原理**：不引入独立的分布式缓存集群，直接通过对象存储（如 S3）的客户端 SDK 或 FUSE 驱动发起高并发的分片流式拉取。
*   **优劣势**：架构极度轻巧，免去了庞大存储集群的容量规划与运维；但完全抛弃了跨节点的 P2P 共享能力。
*   **场景考量**：这一流派常见于顶级公有云环境（如 AWS、GCP）。得益于云厂商极其强悍的基础设施以及对象存储数十 GB/s 的网络带宽，它们甚至可以 **“免用本地缓存（No Local Cache）”** 依旧保障极致的加载性能。然而，并不是所有私有化算力中心都具备如此优渥的底座，在网络带宽受限的场景下，缺乏本地缓存与 P2P 机制往往会引发严重的网络雪崩。
