...
2. **并发派发**：AI 网关将带有连接信息（目标节点地址和 Room ID）的请求，**并发地**同时发送给选定的 Prefill 节点 and Decode 节点。
3. **点对点握手与预分配**：
...
#### 第一节：第一性原理：审视分布式推理下的生命周期与 K8s 的鸿沟

为了看清 Kubernetes 在 AI 时代需要做什么，我们必须回归第一性原理，审视分布式推理的物理本质与 K8s 设计哲学之间的根本冲突。

**1. 哲学的冲突：Borg 的遗产与超级计算机的现实**

Kubernetes 的核心设计哲学源于 Google 的 Borg 系统，其假设是：**Workload 是大量独立的、无状态（或弱状态）的、可随时被替换的微服务**。系统追求的是基于声明式状态（Declarative State）的最终一致性，资源被抽象为同质的标量（如 CPU 核数、内存字节）。

然而，大规模分布式 LLM 推理的物理本质截然不同。它是在极其严格的时延限制下，进行**高度确定性、拓扑强依赖、且需要极高速通信（NVLink/RDMA）的紧耦合高性能计算（HPC）任务**。

这种“无状态网络应用”与“紧耦合超级计算机”的错配，在生命周期的每一个环节都撕扯出了巨大的鸿沟（Gaps）。

**2. 深度剖析：生命周期各环节的“范式冲突”**

*   **准入与调度：从“装箱问题”到“组合优化”**
    *   *标量 vs 拓扑*：K8s 假设资源需求是静态声明的标量（我要 8 个 GPU）。但 LLM 需要的是“形状”（Shape/Topology）—— 8 个在同一 NVSwitch domain 内的 GPU，附带 400Gbps RDMA。
    *   *装箱 vs 组合优化*：默认调度器基于 Pod 维度进行 Bin-packing。但 LLM 推理（尤其是 Prefill/Decode 分离架构）需要对多节点 Gang、网络拓扑、甚至 KV Cache 亲和性进行复杂的组合优化求解。
    *   *静态绑定 vs 持续优化*：K8s 假设 Pod 绑定节点后就不再动了。但在 LLM 场景下，调度器需要从“一次性放置决策器”演进为**“持续优化器”**，根据流量动态和 Cache 分布持续重平衡。
    *   *Pending 堆积*：K8s 假设 Admission Webhook 是轻量的。对于动辄需要数十卡 Gang 的 LLM，在 Admission 阶段就需要引入**可行性预检查（Feasibility Check）**，避免不可调度的 Pending Pod 堆积压垮控制面。

*   **启动与预热：被忽视的“Warm”状态**
    *   *冷启动的灾难*：K8s 假设容器拉取和启动是秒级的。但 LLM 加载数百 GB 权重需要数分钟，并发冷启动时极易引发存储系统的 **I/O 风暴**。
    *   *二值状态的局限*：K8s 的 Pod 生命周期只有 Pending 和 Running。但 LLM 在进程启动后，还需要经历模型加载、CUDA graph 编译、NCCL all-reduce warmup 等漫长过程。系统缺乏一个 **"Loaded but not serving" (Warm)** 的中间状态，导致健康检查极易超时，或者过早引入流量导致 TTFT 爆炸。
    *   *权重即资源*：权重应该被系统原生理解为一种具备跨 Pod 共享、层次化缓存（NVMe $ightarrow$ RAM $ightarrow$ GPU HBM）能力的一等公民资源，而非黑盒的数据卷。

*   **运行与扩缩：失效的指标与语义感知**
    *   *连续的健康度*：K8s 假设 Readiness 是二值的（Ready/Not Ready）。但 LLM 实例的健康度是**连续的**（队列深度、KV Cache 占用率、已加载的 LoRA 适配器）。Liveness Probe 的粗暴反应极易导致“抖动误杀”。
    *   *慢半拍的扩容*：HPA 基于 CPU/Mem 指标对 LLM 毫无意义。即使基于并发数，其响应式扩容（Reactive Scaling）也远跟不上流量变化，等 Pod 扩出来，洪峰早已过去。LLM 需要**预测性扩缩容（Predictive Scaling）**。
    *   *无知的路由*：原生 Service 基于 iptables/IPVS，对“哪个实例有更多相关 KV Cache”这种语义一无知。我们需要将路由决策与推理引擎的 Radix Tree 状态绑定，实现基于内容的路由。

*   **故障与升级：脆弱的 Gang 与高昂的代价**
    *   *僵尸群效应*：K8s 假设 Pod 死了原地重启即可。但在分布式推理组中，一个 GPU 掉线或 NCCL hang（进程在，但通信已死锁），会导致整个 TP/PP 组瘫痪。K8s 原生的独立重启机制会导致其他健康的 Pod 在原地“等死”，形成**僵尸群**。
    *   *昂贵的滚动升级*：K8s 的滚动升级假设可以逐个 Pod 替换。但 LLM 的一个副本就是 16 卡甚至更庞大的 Gang，必须整体替换。在升级期间需要维持双倍的昂贵资源，或者承受漫长的服务中断。

**3. 元问题：在 K8s 内部演进，还是在其上造神？**

审视这些 Gap，业界正面临一个战略性的路线抉择：

1.  **上游演进（Upstream Evolution）**：通过 DRA（动态资源分配）、Gateway API Inference Extension、JobSet 等新特性，在 K8s 原生社区逐步填平这些鸿沟。优势是生态统一，劣势是演进缓慢，且受制于 K8s 的根本假设。
2.  **意见层（Opinionated Layer）**：在 K8s 之上构建一个专门面向 LLM 的“薄操作系统”（如 vLLM/SGLang 的集群管理器，或者类似 Ray 的框架），将 K8s 仅作为纯粹的“粗粒度资源提供者”，而把 Gang 调度、Cache 路由、故障恢复等重任收拢到这个上层。

对于顶级科技公司或需要交付离线专有云（如 air-gapped 环境）的场景，往往需要一个高度自洽、打破 K8s 部分死板假设的完整 Inference Stack，而不仅仅是社区散装组件的拼凑。\n\n---
